...
/**
 *\fn void PQCLEAN_HQC1922CCA2_LEAKTIME_hqc_pke_keygen(uint8_t* pk, uint8_t* sk)
...
void PQCLEAN_HQC1922CCA2_LEAKTIME_hqc_pke_keygen(uint8_t *pk, uint8_t *sk) {
    ...
    // Compute secret key
    PQCLEAN_HQC1922CCA2_LEAKTIME_vect_fixed_weight(&sk_seedexpander, x, PARAM_OMEGA);
    PQCLEAN_HQC1922CCA2_LEAKTIME_vect_fixed_weight(&sk_seedexpander, y, PARAM_OMEGA);

    // Compute public key
    PQCLEAN_HQC1922CCA2_LEAKTIME_vect_set_random(&pk_seedexpander, h);
    PQCLEAN_HQC1922CCA2_LEAKTIME_ntl_cyclic_product(s, h, y);
    PQCLEAN_HQC1922CCA2_LEAKTIME_vect_add(s, s, x, VEC_N_SIZE_BYTES);
...
/**
 *\fn void PQCLEAN_HQC1922CCA2_LEAKTIME_hqc_pke_encrypt(uint8_t* u, uint8_t* v, uint8_t* m, uint8_t* theta, const uint8_t* pk)
...
void PQCLEAN_HQC1922CCA2_LEAKTIME_hqc_pke_encrypt(uint8_t *u, uint8_t *v, const uint8_t *m, const uint8_t *theta, const uint8_t *pk) {
    ...
    PQCLEAN_HQC1922CCA2_LEAKTIME_vect_fixed_weight(&seedexpander, r1, PARAM_OMEGA_R);
    PQCLEAN_HQC1922CCA2_LEAKTIME_vect_fixed_weight(&seedexpander, r2, PARAM_OMEGA_R);
    PQCLEAN_HQC1922CCA2_LEAKTIME_vect_fixed_weight(&seedexpander, e, PARAM_OMEGA_E);

    // Compute u = r1 + r2.h
    PQCLEAN_HQC1922CCA2_LEAKTIME_ntl_cyclic_product(u, r2, h);
    PQCLEAN_HQC1922CCA2_LEAKTIME_vect_add(u, u, r1, VEC_N_SIZE_BYTES);
    ...
    PQCLEAN_HQC1922CCA2_LEAKTIME_ntl_cyclic_product(tmp2, r2, s);

    PQCLEAN_HQC1922CCA2_LEAKTIME_vect_add(tmp2, tmp2, e, VEC_N_SIZE_BYTES);
...
void PQCLEAN_HQC1922CCA2_LEAKTIME_hqc_pke_decrypt(uint8_t *m, const uint8_t *u, const uint8_t *v, const uint8_t *sk) {
    ...
    PQCLEAN_HQC1922CCA2_LEAKTIME_ntl_cyclic_product(tmp2, y, u);
    PQCLEAN_HQC1922CCA2_LEAKTIME_vect_add(tmp2, tmp1, tmp2, VEC_N_SIZE_BYTES);
